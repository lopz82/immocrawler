...
from immocrawler.config import BASE_URL, PAGE_NUMBER_SLUG, MAIN_LINK
from immocrawler.utils import get_logger, random_time
from immocrawler.webdrivers import RequestsDriver
...
class ImmobilienScoutSearch:
    def __init__(self, wait: Tuple[int] = None, rooms: int = None, price: float = None,
        ...
        self.driver = RequestsDriver()
        self.rooms = rooms
        self.price = price
        self.living_space = living_space
        self.url = BASE_URL + self.add_params_to_url()
        self.soup = self.crawl(self.url)
        self.n_pages = None
    ...
    def crawl(self, url) -> BeautifulSoup:
        return BeautifulSoup(self.driver.html_from(url), "html.parser")
    ...
    def get_target_links_from(self, url: str) -> None:
        ...
        matches = html.select(MAIN_LINK)
        links = [match["href"] for match in matches]
